...
class PanelExchanger:
    ...
    def decode_panel_link(link):
        try:
            ...
            if 'share' in query:
                encoded = query['share'][0]
                try:
                    padded = encoded.replace('-', '+').replace('_', '/')
                    padded += '=' * ((4 - len(padded) % 4) % 4)
                    record = json.loads(base64.b64decode(padded).decode('utf-8'))
                    if (record.get('v') == 1 and record.get('name') and
                            record.get('url') and record.get('key')):
                        firebase_data['database_url'] = str(record['url']).strip()
                        firebase_data['api_key'] = str(record['key']).strip()
    ...
    def generate_encoded_link(firebase_data):
        """Build the current ERA X panel share link: ?share=<v1 JSON token>."""
        db_url = PanelExchanger._db_url_of(firebase_data)
        api_key = firebase_data.get('api_key') or FIREBASE_ONLY_API_FALLBACK
        if not db_url:
            return None
        db_url = str(db_url).strip().rstrip('/')
        # The updated HTML panel requires non-empty v1 name, url and key fields.
        project_id = str(firebase_data.get('project_id') or '').strip()
        name = project_id or db_url.split('//', 1)[-1].split('.', 1)[0] or 'ERA'
        record = {'v': 1, 'name': name, 'url': db_url, 'key': str(api_key).strip() or FIREBASE_ONLY_API_FALLBACK}
        payload = json.dumps(record, ensure_ascii=False, separators=(',', ':')).encode('utf-8')
        encoded = base64.urlsafe_b64encode(payload).decode('ascii').rstrip('=')
        return f"{PANEL_URL}?share={quote(encoded, safe='')}"
